...
advancedAdminRouter.get("/notifications/manage", async (_req, res) => { try { const items = await AdminNotification.find().sort({ createdAt: -1 }).limit(100).populate("createdBy", "name email").lean(); return res.json({ items }); } catch { return res.status(500).json({ message: "Unable to load notifications" }); } });
